...
4.  Perform ridge regression on the training set with parameter tuning (choose a reasonable grid for lambda) using k-fold validation. Report the best value of lambda, then generate predictions on the test set, and report the parameter values and the rmse on the test set.
...
8.  Perform LASSO regression on the training set with parameter tuning (choose a reasonable grid for lambda) using k-fold validation. Report the best value of lambda, then generate predictions on the test set, and report the parameter values and the rmse on the test set.
